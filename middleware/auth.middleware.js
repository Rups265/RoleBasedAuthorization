...
const { headerDecode } = require("../utils/helpers/common.utils");
...
class JWT {
    ...
    try {
      if (!req.session) {
        return res.status(400).json({
          message: "logged out please login first",
          status: "failed",
          data: null,
          code: 201,
        });
      ...
      console.log("sssfffffdsgff", decodeOuth);
  ...
  async authenticateJWT(req, res, next) {
    try {
      console.log("dsfaaaaaaaaaa", req.session);
      if (!req.session) {
        return res.status(400).json({
          message: "logged out please login first",
          status: "failed",
